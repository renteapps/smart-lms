    ...
    <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
      <aside className="space-y-5">
        <div className="rounded-2xl border border-accent/25 bg-accent/5 p-5">
          ...
          <h3 className="mt-3 text-xl font-bold text-foreground">Simule um perfil real</h3>
          <p className="mt-2 text-sm leading-6 text-muted">A prévia usa as mesmas regras do aluno e explica por que cada conteúdo entrou.</p>
        </div>
        {contentQuestions.map((question) => (
          <fieldset key={question.id} className="rounded-xl border border-border bg-surface p-4">
            <legend className="px-1 text-sm font-bold text-foreground">{question.text}</legend>
            <div className="mt-3 flex flex-wrap gap-2">
              {question.options.map((option) => {
                const selected = answers[question.id]?.includes(option.label);
                return <button key={option.label} type="button" aria-pressed={selected} onClick={() => toggleAnswer(question.id, option.label, question.type === 'single')} className={cn('rounded-lg border px-3 py-2 text-xs font-semibold', selected ? 'border-accent bg-accent-soft text-accent-soft-foreground' : 'border-border text-muted')}>{option.label}</button>;
              })}
            </div>
          </fieldset>
        ))}
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 sm:p-6">
        <div className="flex items-center justify-between gap-4 border-b border-border pb-4"><div><p className="eyebrow">Agenda calculada</p><h3 className="mt-1 text-2xl font-extrabold text-foreground">{Object.keys(sessions).length} sessões · {trail.items.length} conteúdos</h3></div><CalendarDays className="h-7 w-7 text-accent" /></div>
        {trail.items.length === 0 ? <p className="py-16 text-center text-sm text-muted">Selecione respostas com conteúdos associados para gerar a simulação.</p> : (
          <div className="mt-6 space-y-7">{Object.entries(sessions).map(([sessionId, items]) => (
            <div key={sessionId}>
              ...
              <div className="space-y-2">{items.map((item) => <article key={item.id} className="rounded-lg border border-border bg-background p-4"><div className="flex items-start justify-between gap-3"><div><span className="text-3xs font-extrabold uppercase tracking-[0.1em] text-accent-soft-foreground">{roleLabels[item.learningRole]}</span><h4 className="mt-1 font-bold text-foreground">{item.title}</h4><p className="mt-1 text-xs text-muted">{item.reason}</p></div><span className="shrink-0 text-xs font-bold text-muted">{item.durationMin} min</span></div>{item.overBudget && <p className="mt-3 flex items-center gap-1.5 text-xs font-semibold text-warning"><TriangleAlert size={14} /> {LONGER_CONTENT_LABEL}: maior que a meta do dia, fica sozinho nesta sessão.</p>}</article>)}</div>
            </div>
          ))}</div>
        )}
      </section>
