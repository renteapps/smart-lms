...
export const StepWizard: React.FC<StepWizardProps> = ({ steps, currentStep, onStepClick }) => {
  return (
    <div className="w-full bg-surface/90 backdrop-blur-md border border-border/50 rounded-2xl p-4 sm:p-6 shadow-md mb-8 sticky top-4 z-30 transition-all">
          ...
          return (
            ...
            >
              ...
              <div className="overflow-hidden">
                <span className={`block font-bold text-sm truncate ${isCurrent ? 'text-accent' : 'text-foreground'}`}>
                  {step.title}
                </span>
                <span className="block text-xs text-muted truncate">{step.subtitle}</span>
              </div>
            </button>
          );
        })}
