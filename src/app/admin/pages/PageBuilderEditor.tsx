...
function SectionFields({ section, catalog, onPatch }: { section: PageSection; catalog: PageBuilderData; onPatch: (patch: Partial<PageSection>) => void }) {
  ...
  return <>
    <div className="grid grid-cols-2 gap-3">
      <Field label="Fundo"><NativeSelect value={section.style.background} onChange={(event) => onPatch({ style: { ...section.style, background: event.target.value as typeof section.style.background } } as Partial<PageSection>)}><option value="default">Padrão</option><option value="muted">Suave</option><option value="accent">Cor da marca</option><option value="dark">Escuro</option></NativeSelect></Field>
      {section.type !== "gallery-course-carousel" && (
        <Field label="Largura"><NativeSelect value={section.style.width} onChange={(event) => onPatch({ style: { ...section.style, width: event.target.value as typeof section.style.width } } as Partial<PageSection>)}><option value="narrow">Estreita</option><option value="normal">Normal</option><option value="wide">Ampla</option></NativeSelect></Field>
      )}
      <Field label="Espaçamento"><NativeSelect value={section.style.spacing} onChange={(event) => onPatch({ style: { ...section.style, spacing: event.target.value as typeof section.style.spacing } } as Partial<PageSection>)}><option value="compact">Compacto</option><option value="normal">Normal</option><option value="spacious">Amplo</option></NativeSelect></Field>
      <Field label="Alinhamento"><NativeSelect value={section.style.alignment} onChange={(event) => onPatch({ style: { ...section.style, alignment: event.target.value as typeof section.style.alignment } } as Partial<PageSection>)}><option value="left">Esquerda</option><option value="center">Centro</option></NativeSelect></Field>
    </div>
    {section.type === "gallery-course-carousel" && (
...
function HeroFields({ section, onPatch }: { section: Extract<PageSection, { type: "hero" }>; onPatch: (patch: Partial<PageSection>) => void }) {
  ...
  return <>
    <Field label="Mídia"><NativeSelect value={mediaType} onChange={(event) => onPatch({ media: event.target.value === "none" ? undefined : { type: event.target.value as "image" | "video", url: "", provider: event.target.value === "video" ? "youtube" : undefined } } as Partial<PageSection>)}><option value="none">Sem mídia</option><option value="image">Imagem</option><option value="video">Vídeo</option></NativeSelect></Field>
    ...
    {section.media?.type === "video" && <><Field label="Provedor"><NativeSelect value={section.media.provider ?? "youtube"} onChange={(event) => onPatch({ media: { ...section.media!, provider: event.target.value as "youtube" | "panda" | "direct" } } as Partial<PageSection>)}><option value="youtube">YouTube</option><option value="panda">PandaVideo</option><option value="direct">URL/embed</option></NativeSelect></Field><Field label="URL do vídeo"><input className={inputClass} value={section.media.url} onChange={(event) => onPatch({ media: { ...section.media!, url: event.target.value } } as Partial<PageSection>)} /></Field></>}
...
function CtaEditor({ value, onChange }: { value?: PageCta; onChange: (value: PageCta | undefined) => void }) {
  ...
  return <div className="grid gap-2 rounded-xl border border-border bg-default p-3 sm:grid-cols-[1fr_1.2fr_auto_auto]">
    ...
    <NativeSelect aria-label="Estilo do botão" value={value.variant} onChange={(event) => onChange({ ...value, variant: event.target.value as typeof value.variant })}><option value="primary">Primário</option><option value="secondary">Secundário</option></NativeSelect>
...
function VideoFields({ section, onPatch }: { section: Extract<PageSection, { type: "video" }>; onPatch: (patch: Partial<PageSection>) => void }) {
  return <><Field label="Provedor"><NativeSelect value={section.provider} onChange={(event) => onPatch({ provider: event.target.value as typeof section.provider, url: "" } as Partial<PageSection>)}><option value="youtube">YouTube</option><option value="panda">PandaVideo</option><option value="direct">URL/embed</option></NativeSelect></Field>
...
function SourceEditor({ section, catalog, onPatch }: { section: Extract<PageSection, { source: ContentSource }>; catalog: PageBuilderData; onPatch: (patch: Partial<PageSection>) => void }) {
  ...
  return <div className="space-y-4 rounded-xl border border-border bg-default p-4">
    <div className="grid grid-cols-2 gap-3"><Field label="Fonte"><NativeSelect value={source.mode} onChange={(event) => patchSource({ mode: event.target.value as ContentSource["mode"] })}><option value="automatic">Automática</option><option value="manual">Manual</option></NativeSelect></Field><Field label="Quantidade"><input className={inputClass} type="number" min={1} max={20} value={source.limit} onChange={(event) => patchSource({ limit: Math.max(1, Math.min(20, Number(event.target.value) || 1)) })} /></Field></div>
    {source.mode === "automatic" ? <div className="grid grid-cols-2 gap-3"><Field label="Regra"><NativeSelect value={source.rule} onChange={(event) => patchSource({ rule: event.target.value as ContentSource["rule"] })}><option value="all">Todos</option>{section.type !== "profile-test-carousel" && <option value="featured">Destaques</option>}<option value="recent">Mais recentes</option>{categories.length > 0 && <option value="category">Categoria</option>}</NativeSelect></Field>{source.rule === "category" && <Field label="Categoria"><NativeSelect value={source.category ?? ""} onChange={(event) => patchSource({ category: event.target.value })}><option value="">Selecione</option>{categories.map((category) => <option key={category}>{category}</option>)}</NativeSelect></Field>}</div> : <ManualPicker options={options} selected={source.itemIds} onChange={(itemIds) => patchSource({ itemIds })} />}
  </div>;
}
