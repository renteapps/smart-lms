...
import { Button, Card, ProgressBar, Table, Tabs } from "@heroui/react";
    ...
    <div className="space-y-8">
      ...
      <Tabs.Root defaultSelectedKey="overview">
        ...
        <Tabs.Panel id="simulator" className="pt-5">
          <Card><Card.Header><Card.Title>Simulador antes de salvar</Card.Title><Card.Description>Compare custo do provedor e débito do usuário com a regra atual.</Card.Description></Card.Header><Card.Content className="gap-5">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <label className="space-y-1.5"><span className="text-xs font-semibold text-muted">Função</span><NativeSelect value={simFeature} onChange={(event) => setSimFeature(event.target.value)}>{policies.map((policy) => <option key={policy.feature} value={policy.feature}>{policy.display_name}</option>)}</NativeSelect></label>
              <label className="space-y-1.5"><span className="text-xs font-semibold text-muted">Modelo</span><NativeSelect value={simModel} onChange={(event) => setSimModel(event.target.value)}>{models.map((model) => <option key={model.model} value={model.model}>{model.display_name}</option>)}</NativeSelect></label>
        ...
        <Tabs.Panel id="usage" className="space-y-4 pt-5">
          <div className="flex flex-wrap gap-3"><NativeSelect className="max-w-56" value={featureFilter} onChange={(event) => { setFeatureFilter(event.target.value); setPage(1); }}><option value="all">Todas as funções</option>{policies.map((policy) => <option key={policy.feature} value={policy.feature}>{policy.display_name}</option>)}</NativeSelect><NativeSelect className="max-w-48" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setPage(1); }}><option value="all">Todos os status</option><option value="settled">Liquidado</option><option value="reserved">Reservado</option><option value="refunded">Estornado</option><option value="failed">Falhou</option></NativeSelect><NativeSelect aria-label="Agrupar consumo por" className="max-w-48" value={groupBy} onChange={(event) => setGroupBy(event.target.value as typeof groupBy)}><option value="feature">Agrupar por função</option><option value="user">Agrupar por usuário</option><option value="model">Agrupar por modelo</option></NativeSelect></div>
