  ...
  return (
    <div className="space-y-8">
        ...
        <Card.Content className="gap-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted">Real {money(monthCost)} + reservado {money(monthReserved)} de {money(settings.monthlyBudgetBrl)}</p>
            </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          [Banknote, "Custo real", money(totals.cost), `${totals.calls} chamadas liquidadas`],
          [Coins, "Créditos cobrados", integer(totals.credits), `${money(totals.revenue)} nominais`],
      ...
      <Tabs.Root defaultSelectedKey="overview">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <Card><Card.Header><Card.Title>Consumo por função</Card.Title><Card.Description>Custo real no período selecionado.</Card.Description></Card.Header><Card.Content className="gap-3">
              {policies.map((policy) => {
                ...
                return <div key={policy.feature} className="flex items-center justify-between rounded-xl bg-background-secondary p-3"><span className="text-sm font-semibold">{policy.display_name}</span><span className="font-bold">{money(cost)}</span></div>;
              })}
            </Card.Content></Card>
            <Card><Card.Header><Card.Title>Projeção mensal</Card.Title><Card.Description>Ritmo atual comparado ao teto da plataforma.</Card.Description></Card.Header><Card.Content>
              <p className="text-3xl font-bold">{money(projectedMonthCost)}</p><p className="mt-2 text-sm text-muted">Projeção no ritmo atual; custo realizado {money(monthCost)}. O bloqueio é automático em {money(settings.monthlyBudgetBrl)} e reservas simultâneas também entram na proteção.</p>
        ...
        <Tabs.Panel id="limits" className="space-y-5 pt-5">
          <Card><Card.Header><Card.Title>Orçamento e franquia padrão</Card.Title></Card.Header><Card.Content className="gap-5"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            ...
            <Field label="Padrão por dia" value={settings.defaultDailyCredits} onChange={(value) => setSettings({ ...settings, defaultDailyCredits: value })} />
            <Field label="Padrão por semana" value={settings.defaultWeeklyCredits} onChange={(value) => setSettings({ ...settings, defaultWeeklyCredits: value })} />
            <Field label="Padrão por mês" value={settings.defaultMonthlyCredits} onChange={(value) => setSettings({ ...settings, defaultMonthlyCredits: value })} />
