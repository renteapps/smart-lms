...
select extensions.is((select daily_used from public.ai_credit_accounts where user_id = '44000000-0000-0000-0000-000000000001'), 5::numeric, 'reserva reduz a franquia imediatamente');
select extensions.is((select amount_credits from public.ai_credit_ledger where usage_event_id = (select id from public.ai_usage_events where request_key = '44000000-0000-0000-0000-000000000011') and entry_type = 'reservation'), (-5)::numeric, 'ledger registra a reserva');
...
select extensions.is((select credits_charged from public.ai_usage_events where request_key = '44000000-0000-0000-0000-000000000011'), 3::numeric, 'liquidação grava custo real');
select extensions.is((select daily_used from public.ai_credit_accounts where user_id = '44000000-0000-0000-0000-000000000001'), 3::numeric, 'liquidação devolve a diferença reservada');
select extensions.is((select amount_credits from public.ai_credit_ledger where usage_event_id = (select id from public.ai_usage_events where request_key = '44000000-0000-0000-0000-000000000011') and entry_type = 'refund'), 2::numeric, 'ledger registra estorno parcial');
...
select extensions.is((public.cancel_ai_usage((select id from public.ai_usage_events where request_key = '44000000-0000-0000-0000-000000000012'), 'provider_error')->>'refunded_credits')::numeric, 4::numeric, 'falha estorna toda a reserva');
...
select extensions.is((select daily_used from public.ai_credit_accounts where user_id = '44000000-0000-0000-0000-000000000001'), 3::numeric, 'estorno integral restaura a franquia');
...
select extensions.is((select reservation_credits from public.ai_usage_events where request_key = '44000000-0000-0000-0000-000000000013'), 0::numeric, 'sandbox não debita o administrador');
...
select extensions.throws_ok(
  ...
  '23514', null, 'ledger não permite alteração'
);
