-- Controle financeiro e franquias de IA baseados no custo real do provedor.
-- Os campos ai_* legados em profiles permanecem por compatibilidade, mas deixam
-- de ser a fonte de verdade a partir desta migração.

alter table public.plans
  add column if not exists ai_daily_credits bigint not null default 25,
  add column if not exists ai_weekly_credits bigint not null default 100,
  add column if not exists ai_monthly_credits bigint not null default 400;

alter table public.plans
  add constraint plans_ai_daily_credits_positive check (ai_daily_credits > 0),
  add constraint plans_ai_weekly_credits_positive check (ai_weekly_credits > 0),
  add constraint plans_ai_monthly_credits_positive check (ai_monthly_credits > 0),
  add constraint plans_ai_credit_limits_ordered check (ai_daily_credits <= ai_weekly_credits and ai_weekly_credits <= ai_monthly_credits);

update public.plans
set
  ai_daily_credits = 25,
  ai_weekly_credits = 100,
  ai_monthly_credits = 400,
  features = coalesce(features, '{}'::jsonb) - 'aiTokensUnlimited' - 'aiTokensWeekly';

create table public.ai_billing_settings (
  id smallint primary key default 1 check (id = 1),
  credit_value_brl numeric(12,6) not null default 0.01 check (credit_value_brl > 0),
  target_margin_percent numeric(6,3) not null default 50 check (target_margin_percent >= 0 and target_margin_percent < 100),
  exchange_buffer_percent numeric(6,3) not null default 10 check (exchange_buffer_percent >= 0 and exchange_buffer_percent <= 100),
  reservation_buffer_percent numeric(6,3) not null default 25 check (reservation_buffer_percent >= 0 and reservation_buffer_percent <= 200),
  monthly_budget_brl numeric(14,4) not null default 500 check (monthly_budget_brl > 0),
  warning_threshold_percent numeric(6,3) not null default 70 check (warning_threshold_percent > 0 and warning_threshold_percent < 100),
  critical_threshold_percent numeric(6,3) not null default 90 check (critical_threshold_percent > 0 and critical_threshold_percent < 100),
  default_daily_credits bigint not null default 25 check (default_daily_credits > 0),
  default_weekly_credits bigint not null default 100 check (default_weekly_credits > 0),
  default_monthly_credits bigint not null default 400 check (default_monthly_credits > 0),
  manual_exchange_rate numeric(12,6) check (manual_exchange_rate is null or manual_exchange_rate > 0),
  monthly_reserved_cost_brl numeric(14,6) not null default 0 check (monthly_reserved_cost_brl >= 0),
  monthly_actual_cost_brl numeric(14,6) not null default 0 check (monthly_actual_cost_brl >= 0),
  budget_period_started_at timestamptz not null default (
    date_trunc('month', timezone('America/Fortaleza', now())) at time zone 'America/Fortaleza'
  ),
  updated_by uuid references auth.users(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ai_billing_default_limits_ordered check (
    default_daily_credits <= default_weekly_credits and default_weekly_credits <= default_monthly_credits
  )
);

insert into public.ai_billing_settings (id) values (1) on conflict (id) do nothing;

create table public.ai_feature_policies (
  feature text primary key check (feature in ('agent_chat', 'platform_assistant', 'admin_sandbox')),
  display_name text not null,
  enabled boolean not null default true,
  charge_user boolean not null default true,
  margin_override_percent numeric(6,3) check (margin_override_percent is null or (margin_override_percent >= 0 and margin_override_percent < 100)),
  minimum_credits bigint not null default 1 check (minimum_credits > 0),
  max_output_tokens integer not null default 1500 check (max_output_tokens between 1 and 32000),
  updated_at timestamptz not null default now()
);

insert into public.ai_feature_policies (feature, display_name, charge_user)
values
  ('agent_chat', 'Agentes de IA', true),
  ('platform_assistant', 'Assistente da plataforma', true),
  ('admin_sandbox', 'Sandbox administrativo', false)
...
create table public.ai_exchange_rates (
  ...
  source text not null check (source in ('bcb_ptax', 'manual')),
  fetched_at timestamptz not null default now(),
  unique (rate_date, source)
);

create index ai_exchange_rates_latest_idx on public.ai_exchange_rates (rate_date desc, fetched_at desc);

create table public.ai_model_pricing (
  model text primary key,
  display_name text not null,
  prompt_usd_per_million numeric(18,8) not null check (prompt_usd_per_million >= 0),
  completion_usd_per_million numeric(18,8) not null check (completion_usd_per_million >= 0),
  enabled boolean not null default true,
...
insert into public.ai_model_pricing (model, display_name, prompt_usd_per_million, completion_usd_per_million)
values
  ...
  ('meta-llama/llama-3.3-70b-instruct', 'Llama 3.3 70B Instruct', 0.12, 0.30),
  ('mistralai/mistral-large-2411', 'Mistral Large', 2.00, 6.00)
on conflict (model) do nothing;

create table public.ai_credit_accounts (
  user_id uuid primary key references auth.users(id) on delete cascade,
  daily_limit bigint not null default 25 check (daily_limit > 0),
  weekly_limit bigint not null default 100 check (weekly_limit > 0),
  monthly_limit bigint not null default 400 check (monthly_limit > 0),
  daily_used bigint not null default 0 check (daily_used >= 0),
  weekly_used bigint not null default 0 check (weekly_used >= 0),
  monthly_used bigint not null default 0 check (monthly_used >= 0),
  extra_balance bigint not null default 0 check (extra_balance >= 0),
  daily_period_started_at timestamptz not null default (
    date_trunc('day', timezone('America/Fortaleza', now())) at time zone 'America/Fortaleza'
  ),
  weekly_period_started_at timestamptz not null default (
    date_trunc('week', timezone('America/Fortaleza', now())) at time zone 'America/Fortaleza'
  ),
  monthly_period_started_at timestamptz not null default (
    date_trunc('month', timezone('America/Fortaleza', now())) at time zone 'America/Fortaleza'
  ),
  updated_at timestamptz not null default now(),
  constraint ai_credit_account_limits_ordered check (
    daily_limit <= weekly_limit and weekly_limit <= monthly_limit
  )
);
...
set
  ai_credits = 0,
  ai_weekly_credits_used = 0,
  ai_monthly_credits_used = 0;

create table public.ai_usage_events (
  id uuid primary key default gen_random_uuid(),
  request_key uuid not null unique,
  user_id uuid references auth.users(id) on delete set null,
  feature text not null references public.ai_feature_policies(feature),
  model text not null,
  status text not null default 'reserved' check (status in ('reserved', 'settled', 'refunded', 'failed')),
  charge_user boolean not null,
  reservation_credits bigint not null default 0 check (reservation_credits >= 0),
  reserved_recurring_credits bigint not null default 0 check (reserved_recurring_credits >= 0),
  reserved_extra_credits bigint not null default 0 check (reserved_extra_credits >= 0),
  ...
  provider_cost_brl numeric(18,8) not null default 0 check (provider_cost_brl >= 0),
  protected_cost_brl numeric(18,8) not null default 0 check (protected_cost_brl >= 0),
  nominal_revenue_brl numeric(18,8) not null default 0 check (nominal_revenue_brl >= 0),
  estimated_provider_cost_brl numeric(18,8) not null default 0 check (estimated_provider_cost_brl >= 0),
  exchange_rate numeric(12,6) not null check (exchange_rate > 0),
  prompt_usd_per_million numeric(18,8) not null check (prompt_usd_per_million >= 0),
  completion_usd_per_million numeric(18,8) not null check (completion_usd_per_million >= 0),
  credit_value_brl numeric(12,6) not null check (credit_value_brl > 0),
  margin_percent numeric(6,3) not null check (margin_percent >= 0 and margin_percent < 100),
  exchange_buffer_percent numeric(6,3) not null check (exchange_buffer_percent >= 0),
  pricing_source text not null default 'provider' check (pricing_source in ('provider', 'estimated')),
  generation_id text,
  error_code text,
...
create index ai_usage_events_model_created_idx on public.ai_usage_events (model, created_at desc);
...
create table public.ai_credit_ledger (
  ...
  entry_type text not null check (entry_type in ('reservation', 'refund', 'grant', 'adjustment')),
...
create index ai_credit_ledger_usage_event_idx on public.ai_credit_ledger (usage_event_id) where usage_event_id is not null;
...
as $$
begin
  raise check_violation using message = 'O ledger de créditos de IA é imutável.';
end;
$$;
...
revoke all on function public.prevent_ai_credit_ledger_mutation() from public, anon, authenticated;
...
revoke all on table public.ai_billing_settings, public.ai_feature_policies,
  public.ai_exchange_rates, public.ai_model_pricing, public.ai_credit_accounts,
  public.ai_usage_events, public.ai_credit_ledger from public, anon, authenticated;
grant all on table public.ai_billing_settings, public.ai_feature_policies,
  public.ai_exchange_rates, public.ai_model_pricing, public.ai_credit_accounts,
  public.ai_usage_events to service_role;
grant select, insert on table public.ai_credit_ledger to service_role;
grant usage, select on sequence public.ai_exchange_rates_id_seq, public.ai_credit_ledger_id_seq to service_role;
...
declare
  ...
  resolved_daily bigint;
  resolved_weekly bigint;
  resolved_monthly bigint;
  recurring_available bigint;
  recurring_reserved bigint := 0;
  extra_reserved bigint := 0;
begin
  ...
  set
    daily_used = account.daily_used + case when p_charge_user then p_reservation_credits else 0 end,
...
create or replace function public.refresh_ai_billing_period()
returns jsonb
language plpgsql
volatile
security definer
...
begin
  ...
  if settings.budget_period_started_at < month_start then
    update public.ai_billing_settings
    set monthly_reserved_cost_brl = 0,
        monthly_actual_cost_brl = 0,
        budget_period_started_at = month_start,
        updated_at = now()
    where id = 1 returning * into settings;
  end if;
  return jsonb_build_object(
    'period_started_at', settings.budget_period_started_at,
    'actual_cost_brl', settings.monthly_actual_cost_brl,
    'reserved_cost_brl', settings.monthly_reserved_cost_brl
  );
end;
$$;
...
declare
  ...
  recurring_charged bigint;
  extra_charged bigint;
  recurring_refund bigint;
  extra_refund bigint;
  total_refund bigint;
  remaining bigint;
begin
  ...
  if event.status = 'settled' then
    select least(
      ...
      greatest(weekly_limit - weekly_used, 0) + extra_balance,
      greatest(monthly_limit - monthly_used, 0) + extra_balance
    ) into remaining from public.ai_credit_accounts where user_id = event.user_id;
    return jsonb_build_object(
  ...
  update public.ai_credit_accounts
  set
    daily_used = greatest(daily_used - total_refund, 0),
  ...
  from public.ai_credit_accounts where user_id = event.user_id;

  return jsonb_build_object('credits_charged', p_credits_charged, 'credits_remaining', coalesce(remaining, 0), 'refunded_credits', total_refund);
end;
$$;
...
begin
  ...
  update public.ai_credit_accounts
  set
    daily_used = greatest(daily_used - event.reservation_credits, 0),
...
begin
  ...
  return jsonb_build_object(
    ...
    'available_credits', least(
      ...
      greatest(account.weekly_limit - week_used, 0) + account.extra_balance,
      greatest(account.monthly_limit - month_used, 0) + account.extra_balance
    ),
...
begin
  ...
  if p_amount <= 0 or p_amount > 1000000 then raise check_violation using message = 'Quantidade inválida.'; end if;

  insert into public.ai_credit_accounts (user_id, extra_balance)
  values (p_user_id, p_amount)
  on conflict (user_id) do update set extra_balance = public.ai_credit_accounts.extra_balance + p_amount, updated_at = now();

  insert into public.ai_credit_ledger (user_id, entry_type, amount_credits, note, created_by)
  values (p_user_id, 'grant', p_amount, 'Créditos extras concedidos pelo administrador', caller_id);

  return public.get_ai_credit_balance(p_user_id);
end;
$$;

revoke all on function public.reserve_ai_usage(uuid, text, text, uuid, numeric, bigint, numeric, boolean) from public, anon, authenticated;
revoke all on function public.refresh_ai_billing_period() from public, anon, authenticated;
revoke all on function public.settle_ai_usage(uuid, bigint, numeric, numeric, numeric, bigint, bigint, bigint, bigint, text, text, jsonb) from public, anon, authenticated;
...
grant execute on function public.reserve_ai_usage(uuid, text, text, uuid, numeric, bigint, numeric, boolean) to service_role;
grant execute on function public.refresh_ai_billing_period() to service_role;
grant execute on function public.settle_ai_usage(uuid, bigint, numeric, numeric, numeric, bigint, bigint, bigint, bigint, text, text, jsonb) to service_role;
grant execute on function public.cancel_ai_usage(uuid, text, numeric, numeric, bigint, bigint, text) to service_role;

-- O RPC legado cobrava uma unidade por mensagem e não pode mais ser usado.
...
comment on table public.ai_usage_events is 'Ledger operacional de reservas, custos reais e créditos cobrados por chamada de IA.';
comment on table public.ai_credit_ledger is 'Movimentações imutáveis de créditos: reservas, devoluções e concessões administrativas.';
