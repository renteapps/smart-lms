-- Créditos de IA passam a ser fracionários (numeric(14,4)) em vez de inteiros,
-- permitindo que mensagens baratas custem menos que 1 crédito (inclusive 0).

alter table public.plans
  alter column ai_daily_credits type numeric(14,4),
  alter column ai_weekly_credits type numeric(14,4),
  alter column ai_monthly_credits type numeric(14,4);

alter table public.ai_billing_settings
  alter column default_daily_credits type numeric(14,4),
  alter column default_weekly_credits type numeric(14,4),
  alter column default_monthly_credits type numeric(14,4);

alter table public.ai_feature_policies
  alter column minimum_credits type numeric(14,4),
  alter column minimum_credits set default 0;

alter table public.ai_feature_policies
  drop constraint ai_feature_policies_minimum_credits_check,
  add constraint ai_feature_policies_minimum_credits_check check (minimum_credits >= 0);

-- Sem isto, as 3 políticas já existentes continuam com o piso antigo de 1 crédito
-- (o novo default só vale para linhas futuras).
update public.ai_feature_policies set minimum_credits = 0;

alter table public.ai_credit_accounts
  alter column daily_limit type numeric(14,4),
  alter column weekly_limit type numeric(14,4),
  alter column monthly_limit type numeric(14,4),
  alter column daily_used type numeric(14,4),
  alter column weekly_used type numeric(14,4),
  alter column monthly_used type numeric(14,4),
  alter column extra_balance type numeric(14,4);

alter table public.ai_usage_events
  alter column reservation_credits type numeric(14,4),
  alter column reserved_recurring_credits type numeric(14,4),
  alter column reserved_extra_credits type numeric(14,4),
  alter column credits_charged type numeric(14,4);

alter table public.ai_credit_ledger
  alter column amount_credits type numeric(14,4);

-- reserve_ai_usage muda de assinatura (p_reservation_credits bigint -> numeric),
-- então precisa DROP + CREATE (CREATE OR REPLACE não troca tipo de parâmetro).
-- As variáveis locais de crédito também mudam para numeric: se ficarem bigint,
-- o PL/pgSQL faz cast implícito e arredonda pra inteiro de novo na atribuição,
-- silenciosamente anulando a mudança.
drop function public.reserve_ai_usage(uuid, text, text, uuid, numeric, bigint, numeric, boolean);
...
-- settle_ai_usage: só p_credits_charged muda (bigint -> numeric). Os 4 parâmetros
-- de tokens (prompt/completion/reasoning/cached) continuam bigint — são contagem
-- de tokens, não créditos.
drop function public.settle_ai_usage(uuid, bigint, numeric, numeric, numeric, bigint, bigint, bigint, bigint, text, text, jsonb);
...
-- get_ai_credit_balance: assinatura não muda (só uuid), mas as variáveis locais
-- que leem as colunas agora numeric também precisam virar numeric.
...
-- cancel_ai_usage, add_ai_credits e refresh_ai_billing_period ficam como estão:
-- cancel_ai_usage só usa event.* via rowtype (adapta sozinho); add_ai_credits
-- é bônus manual do admin e continua inteiro de propósito.
